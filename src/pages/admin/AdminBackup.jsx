...
export default function AdminBackup() {
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      toast.success(t('admin.backup.exportDone', {
        name,
        docs: Object.values(payload.counts || {}).reduce((a, b) => a + b, 0),
      }));
      await loadCounts();
    } catch (err) {
      console.error(err);
      ...
      <GlassCard className="p-5 sm:p-6 space-y-5 border border-brand/20">
        <div className="flex flex-col md:flex-row md:items-start justify-between gap-4">
          ...
          <div className="flex items-center gap-2 text-xs font-semibold text-gray-500">
            ...
            {t('admin.backup.full.clientProcessed')}
          </div>
        </div>
      ...
      <div className="grid lg:grid-cols-2 gap-6">
        ...
        <GlassCard className="p-4 sm:p-5 space-y-4">
          ...
          <button
            ...
            className={`w-full inline-flex items-center justify-center gap-2 px-4 py-3.5 rounded-xl font-bold text-sm text-white disabled:opacity-50 ${
