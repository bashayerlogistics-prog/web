  ...
  return (
    <div className="admin-card-surface glass-card-3d p-4 sm:p-5 md:p-6 flex flex-col sm:flex-row sm:items-start justify-between gap-3 sm:gap-4 w-full relative overflow-hidden">
      ...
      <div className="min-w-0 flex-1 relative">
        <h1 className="admin-heading-gradient text-xl sm:text-2xl md:text-3xl font-black bg-gradient-to-r from-brand via-brand-light to-gold bg-clip-text text-transparent truncate leading-tight">
          {title}
        </h1>
        {showPurpose && purpose ? (
        ...
        {subtitle ? (
          <p className={`admin-text-muted text-xs sm:text-sm font-medium ${showPurpose && purpose ? 'mt-2' : 'mt-1.5'}`}>
            {subtitle}
          </p>
        ) : null}
      </div>
