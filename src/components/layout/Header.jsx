          ...
          <div className="flex items-center gap-0.5 sm:gap-1 lg:gap-1 shrink-0">
            <div className="hidden md:flex items-center gap-0.5 lg:gap-1">
              ...
              <div className="flex items-center gap-0.5 lg:gap-1">
                <NavLoginButton asIcon signedInOnly />
                <NavLoginButton variant="topbar" compact guestOnly />
            ...
            <div className="flex md:hidden items-center gap-1 shrink-0">
              <Link
                to="/cart"
                className="header-icon-btn header-icon-btn--mobile touch-target"
                aria-label="Cart"
                onMouseEnter={() => prefetchRoute('/cart')}
                onTouchStart={() => prefetchRoute('/cart')}
              >
              ...
              {/* Profile only when signed in — guests use bottom-nav / menu login */}
              <NavLoginButton
                asIcon
                signedInOnly
                className="header-icon-btn--mobile touch-target"
              />
          ...
          >
            <div className="flex items-center justify-between border-b border-white/10 p-4 sm:p-5 shrink-0">
              ...
              <button
                type="button"
                onClick={() => setMenuOpen(false)}
                className="text-white hover:text-gold transition-colors p-2.5 min-h-11 min-w-11 rounded-lg hover:bg-white/10 touch-target"
            ...
            <div className="flex flex-col gap-1.5 p-4 sm:p-5 overflow-y-auto flex-1 overscroll-contain">
              {navLinks.map((link) =>
                link.dropdown === 'fleet' ? (
                  <FleetCategoriesDropdown
                    key={link.key}
            ...
            <div className="p-4 sm:p-5 border-t border-white/10 space-y-3 shrink-0 safe-area-pb">
              <div className="flex items-center justify-center gap-3">
                <Link
                  to="/cart"
                  onClick={() => setMenuOpen(false)}
                  className="flex items-center gap-2 px-4 py-2.5 min-h-11 rounded-full border border-white/15 text-white text-sm font-semibold leading-snug hover:bg-white/8 hover:border-gold/30 transition-all"
                >
                  <ShoppingCart className="w-4 h-4" />
                  {t('cart.title')}
                  {cartCount > 0 && (
              ...
              <a
                ...
                className="flex items-center justify-center gap-2 text-white border border-white/15 p-3 min-h-11 rounded-full hover:bg-white/8 hover:border-gold/25 transition-all font-medium text-sm leading-snug"
              >
              ...
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="flex items-center justify-center gap-2 text-white border border-white/15 p-3 min-h-11 rounded-full hover:bg-[#25D366]/10 hover:border-[#25D366]/30 transition-all font-medium text-sm leading-snug"
              >
              ...
              <AppNavLink
                to="#pricing-calculator"
                onClick={() => setMenuOpen(false)}
                className="header-cta-btn w-full justify-center p-3.5 min-h-11 text-sm"
              >
